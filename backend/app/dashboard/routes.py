...
@dashboard_router.get("/admin/nodes/{node_id}/discover")
async def discover_node(
    request: Request,
    node_id: int,
    db: AsyncSession = Depends(get_async_db),
):
    """Discover inference endpoints on a node (returns JSON for AJAX)."""
    user_id = get_session_user_id(request)
    if not user_id:
    ...
    if result is None:
        return JSONResponse(
            status_code=400,
            content={"error": "Node has no sidecar configured or sidecar is unreachable"},
        )
...
@dashboard_router.post("/admin/nodes/{node_id}/accept-endpoint")
async def accept_endpoint(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/admin/nodes?error=Invalid+engine+type:+{engine}", status_code=302
        )

    # Parse gpu_indices from comma-separated string
    parsed_indices = None
    if gpu_indices.strip():
        try:
            parsed_indices = [int(x.strip()) for x in gpu_indices.split(",") if x.strip()]
        except ValueError:
            return RedirectResponse(
                url="/admin/nodes?error=Invalid+GPU+indices+format", status_code=302
            )

    try:
        # Check for duplicates
        existing_name = await crud.get_backend_by_name(db, name)
        if existing_name:
            return RedirectResponse(
                url=f"/admin/nodes?error=Backend+name+already+exists:+{name}", status_code=302
            )
        existing_url = await crud.get_backend_by_url(db, url)
        if existing_url:
            return RedirectResponse(
                url=f"/admin/nodes?error=Backend+URL+already+exists:+{url}", status_code=302
            )
        ...
        return RedirectResponse(
            url=f"/admin/nodes?success=Backend+'{name}'+registered+successfully", status_code=302
        )
    except Exception as e:
        error_msg = str(e).replace(" ", "+")
        return RedirectResponse(url=f"/admin/nodes?error={error_msg}", status_code=302)
